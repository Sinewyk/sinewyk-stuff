...
const compilePath = (pattern: string, options: pathToRegexp.RegExpOptions): CompiledPattern => {
...
const matchPath = (pathname: string, options: MatchPathOptions | string = {}) => {
	...
	return routes.reduce((_, route) => {
		...
		if (didMatch) {
			return (sources: Sources) =>
				route.value({
					Params: concat(xs.of({ ...didMatch.params, pathname }), xs.never()).remember(),
					...sources,
				});
		}
